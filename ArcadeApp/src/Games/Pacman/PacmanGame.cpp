...
void PacmanGame::Update(uint32_t dt)
{
	...
	else if(mGameState == PLAY_GAME)
	{
		...
		for(size_t i = 0; i < NUM_GHOSTS; ++i)
		{
			...
			if(ghost.IsVulnerable() && mPacman.GetEatingBoundingBox().Interscts(ghost.GetBoundingBox()))
			{
				ghost.EatenbyPacman();
				mPacman.AteGhost(ghost.GetPoints());
			}
			else if(ghost.IsAlive() && ghost.GetEatingBoundingBox().Interscts(mPacman.GetBoundingBox()))
			{
				mNumLives--;
...
void PacmanGame::SetupGhosts()
{
	...
	blinky.Init(
			mPacmanSpriteSheet,
			App::Singleton().GetBasePath() + "Assets/Ghost_animations.txt",
			mLevel.GetGhostSpawnPoints()[BLINKY],
			GHOST_MOVEMENT_SPEED,
			true,
			Color::Red()
			);
	...
	blinkyAI.Init(
			mGhosts[BLINKY],
			blinky.GetBoundingBox().GetWidth(),
			BLINKY_SCATTER_POS,
			mLevel.GetGhostSpawnPoints()[PINKY],
			mLevel.GetGhostSpawnPoints()[BLINKY],
			BLINKY
			);
	...
	inky.Init(
			mPacmanSpriteSheet,
			App::Singleton().GetBasePath() + "Assets/Ghost_animations.txt",
			mLevel.GetGhostSpawnPoints()[INKY],
			GHOST_MOVEMENT_SPEED,
			true,
			Color::Cyan()
			);
	...
	clyde.Init(
			mPacmanSpriteSheet,
			App::Singleton().GetBasePath() + "Assets/Ghost_animations.txt",
			mLevel.GetGhostSpawnPoints()[CLYDE],
			GHOST_MOVEMENT_SPEED,
			true,
			Color::Orange()
			);
