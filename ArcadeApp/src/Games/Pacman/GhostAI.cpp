...
void GhostAI::Draw(Screen& screen)
{
	if(mnoptrGhost)
	{
		...
		Color c = Color(
				mnoptrGhost->GetSpriteColor().GetRed(),
...
{
	if(mnoptrGhost && mnoptrGhost->IsReleased() && (lastState == GHOST_STATE_VULNERABLE || lastState == GHOST_STATE_VULNERABLE_ENDING) && !(IsInPen() || WantsToLeavePen()))
	{
